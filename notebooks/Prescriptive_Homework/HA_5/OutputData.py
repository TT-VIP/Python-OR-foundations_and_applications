...
class OutputJob(DataJob):
    ...
    def __init__(self, dataJob):
        super().__init__(dataJob.JobId, [dataJob.ProcessingTimes[i] for i in range(len(dataJob.Operations()))], [dataJob.SetupTimes[i] for i in range(len(dataJob.Operations()))], dataJob.DueDate, dataJob.TardCost)
...
class Solution:
    ...
    def __str__(self):
        return "The permutation " + str([int(i) for i in self.Permutation]) + " results in a Makespan of " + str(self.Makespan)
