...
class ConstructiveHeuristics:
    ...
    def ROS(self, jobList, x, seed):
        ...
        for i in range(x):
            tmpPermutation = numpy.random.permutation(len(jobList)).tolist()
            # initialize Solution            
    ...
    def ShortestProcessingTime(self, jobList, allMachines = False):
        ...
        for i in range(len(jobList)):
            if(allMachines):
                jobPool.append((i,sum(jobList[i].ProcessingTimes[x] for x in range(len(jobList[i].Operations())))))
            else: 
                jobPool.append((i,jobList[i].ProcessingTimes[0]))
